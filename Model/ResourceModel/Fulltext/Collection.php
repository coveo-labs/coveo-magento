...
namespace Coveo\Search\Model\ResourceModel\Fulltext;
...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    {
      $this->_logger->debug('Collection.php getCollectionClone');
    ...
    {
      $this->_logger->debug('Collection.php getSearch');
        if ($this->search === null) {
    ...
    {
      $this->_logger->debug('Collection.php addFieldToFilter');
    ...
    public function addSearchFilter($query)
    {
        ...
        $this->_logger->debug('Collection.php addSearchFilter');
        return $this;
    }
    ...
    /**
     * This is the function containing the edits that allows Coveo and Mageplaza_LayeredNavigation to coexist.
     *
    ...
    {
      $this->_logger->debug('Collection.php renderFilters');
        ...
        if ($this->queryText) {
            ...
            $this->searchCriteriaBuilder->addFilter($this->filterBuilder->create());
            $this->_logger->debug('Collection.php queryText: '.$this->queryText);
          }
    ...
    /**
     * Check if the Coveo search module is enabled
     *
     * @return bool
     */
    protected function isCoveoSearchEnabled()
    {
     ...
     * This is the case when Coveo is enabled and the current request is quick_search_container.
     *
     * @return bool
     */
    protected function shouldLoadEmptySearchResult()
    {
        return $this->isCoveoSearchEnabled() && $this->searchRequestName === 'quick_search_container';
    }

    /**
     * Check if I can join the Magento search results to the current collection.
     *
     * This is the case if the Coveo module is currently disabled
     * OR
     ...
     * are always empty if the Coveo module is enabled, and joining them would give an empty result.
     * At the same time this JOIN must be executed in category pages, even when the Coveo module is enabled.
     *
     * @return bool
     */
    protected function canJoinMagentoSearchResults()
    {
        $isCoveoDisabled = !$this->isCoveoSearchEnabled();
        $isQuickSearchContainer = $this->searchRequestName === 'quick_search_container';

        return $isCoveoDisabled || !$isQuickSearchContainer;
    }
    ...
    public function getFacetedData($field)
    {
        $this->_renderFilters();
        $result = [];
        $this->_logger->debug('Collection.php getFacets');
